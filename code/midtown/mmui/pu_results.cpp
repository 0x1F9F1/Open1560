...
#define SY(VALUE) ((VALUE) / 480.0f)
    ...
    AddText(RaceType.get(), MenuMgr()->GetFont(20), LOC_TEXT("Checkpoint"), MM_TEXT_CENTER, 0, 0);

    RaceName = CreateTextNode(SX(124), SY(141), SX(160), SY(20), 1, BITMAP_TRANSPARENT);
    AddText(RaceName.get(), MenuMgr()->GetFont(20), LOC_TEXT("Tough Turns & a Tunnel"), MM_TEXT_CENTER, 0, 0);

    Message = CreateTextNode(SX(124), SY(161), SX(160), SY(60), 1, BITMAP_TRANSPARENT);
    AddText(Message.get(), MenuMgr()->GetFont(20),
    ...
    f32 text_w = 0.0f;
    f32 text_h = 0.0f;
    Names->GetTextDimensions(MenuMgr()->GetFont(16), LOC_TEXT("Just the Height"), text_w, text_h);
    ...
    for (i32 i = 0; i < 10; ++i)
    {
        ...
        AddText(Names.get(), MenuMgr()->GetFont(20), LOC_TEXT(buffer), 0, SX(0), SY(y));
        AddText(Names.get(), MenuMgr()->GetFont(20), LOC_TEXT(buffer), 0, SX(21), SY(y));
        AddText(Names.get(), MenuMgr()->GetFont(20), LOC_TEXT(buffer), 0, SX(203), SY(y));
    }
